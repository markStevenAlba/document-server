...
if (config.has('server.static')) {
  ...
  for (let i = 0; i < staticContent.length; i++) {
    ...
    console.log(staticContentElem, staticContentElem.path, staticContentElem.options)
...
console.log(cfgSignatureSecret, token)
    // try {
...
app.get('/editor', (req, res) => { // define a handler for editing document
  try {
    ...
    const directUrl1 = req.DocManager.getDownloadUrl(fileName);
  console.log(directUrl1)
    const userAddress = req.DocManager.curUserHostAddress();
...
app.post('/rename', (req, res) => { // define a handler for renaming file
  ...
  console.log(newfilename, 'RENAME')
...
app.post('/historyObj', (req, res) => {
  req.DocManager = new DocManager(req, res);
  console.log(req.DocManager, 'HISTORY')
  const { fileName } = req.body;
